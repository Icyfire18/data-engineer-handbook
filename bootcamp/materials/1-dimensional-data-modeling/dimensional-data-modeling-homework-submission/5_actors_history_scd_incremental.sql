        ...
        *,
        CAST(
            CASE
                ...
                WHEN avg(rating) over (partition by actorid) > 7 THEN 'good'
                WHEN avg(rating) over (partition by actorid) > 6 THEN 'average'
...
Aggregated as (
    SELECT
        Combined.actorid,
        Combined.filmid,
        Combined.current_year,
        _yearly_perf.quality_class,
        Combined.films,
        Combined.is_active_current_year
    FROM Combined
    left join _yearly_perf
        on
            Combined.actorid = _yearly_perf.actorid
            and Combined.current_year = _yearly_perf.current_year
)
